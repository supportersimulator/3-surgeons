...
class SurgeonConfig:
    ...
    def get_api_key(self) -> Optional[str]:
        """Read API key, resolving at CALL TIME (no per-process cache).

        Resolution order — single source of truth, drift-proof:
          1. env var self.api_key_env
          2. (DeepSeek) alt env names: Context_DNA_Deep_Seek, Context_DNA_Deepseek, DEEPSEEK_API_KEY
          3. macOS Keychain (default service, account=current user) — covers daemon/MCP
             processes launched without env exported, and propagates key rotation
             instantly without restart.

        Returns None if no source >= 6 chars. ZSF: keychain errors counted, not silent.
        """
        value = os.environ.get(self.api_key_env) if self.api_key_env else None
        if value and len(value) >= 6:
            return value
        # DeepSeek env aliases
        if self.provider == "deepseek":
            for alt in ("Context_DNA_Deep_Seek", "Context_DNA_Deepseek", "DEEPSEEK_API_KEY"):
                v = os.environ.get(alt)
                if v and len(v) >= 6:
                    return v
        # Keychain fallback — resolved each call, so rotation is hot-reloaded.
        for svc in filter(None, (self.api_key_env, "Context_DNA_Deepseek" if self.provider == "deepseek" else None)):
            try:
                import subprocess
                r = subprocess.run(
                ...
                if r.returncode == 0:
                    ...
                    if k and len(k) >= 6:
                        return k
            except Exception as e:  # ZSF: surface, don't swallow
                os.environ["_3S_KEYCHAIN_ERRORS"] = str(int(os.environ.get("_3S_KEYCHAIN_ERRORS", "0")) + 1)
                os.environ["_3S_KEYCHAIN_LAST_ERR"] = f"{type(e).__name__}: {e}"[:200]
        return None
